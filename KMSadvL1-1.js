...
assert("lion" > "donkey"); //strings are compared lexicographically
assert("penguin" === "tiger", "this is an assertion fail; penguins and tigers are not the same.");
...
// console.log("I better hurry and get to the: " + nextAnimal);
...
var mealsPerDay = [ 5, 4, 3, 6, 2, 4, 3, 4, 5, 1 ],
    ...
    mealAvg = 0,
    mealsTotal = 0;
var mealsSince = " meals since the caretaker took over";
for (var i = 0; i < mealsPerDay.length; i++) {
  var day = i + 1; //day should be one more than the array count
      mealsTotal += mealsPerDay[i]; //total of array values from 0 to i
        mealAvg = (mealsTotal / day);  // basic average
      console.log("On day " + day + " the lion ate an average of " + mealAvg + mealsSince);
        if (mealAvg < 4) {
          tooHungryDay = day;
          break;
        }
        ...
        /*
        //A bit of debug code to test this for both the array above and to test for an array
        // where the average does not dip below 4.
        if (tooHungryDay <= mealsPerDay.length) {
         console.log ("Sadly on day " + day + " the lion ate the caretaker.");
      } else
      {
        console.log("The caretaker took the job seriously and the lion never had to eat them.");
      }
      */
